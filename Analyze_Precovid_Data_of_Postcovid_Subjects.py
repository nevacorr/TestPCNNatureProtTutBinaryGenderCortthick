...
visit = 2
brain_good, all_data, roi_ids = load_genz_data(orig_struct_var, visit, working_dir)

#load brain and behavior data for visit 1
visit = 1
brain_v1, all_v1, roi_v1 = load_genz_data(orig_struct_var, visit, working_dir)
...
fname='{}/visit1_subjects_excluded_from_normative_model_test_set_{}_9_11_13.txt'.format(working_dir, orig_struct_var)
subjects_to_include = pd.read_csv(fname, header=None)
subjects_to_include = pd.concat([subjects_to_include, subjs_in_v2_not_v1])
brain_v1 = brain_v1[brain_v1['participant_id'].isin(subjects_to_include[0])]
all_v1 = all_v1[all_v1['participant_id'].isin(subjects_to_include[0])]
...
# reset indices
brain_v1.reset_index(inplace=True)
all_v1.reset_index(inplace=True, drop=True)
#read agemin and agemax from file
agemin, agemax = read_ages_from_file(orig_struct_var)

#replace gender with binary gender
all_v1.loc[all_v1['sex']==2, 'sex'] = 0

#show number of subjects by gender and age
if show_nsubject_plots:
    plot_num_subjs(all_v1, 'Subjects with Post-COVID Data\nEvaluated by Model\n'
                   +' (Total N=' + str(all_v1.shape[0]) + ')', orig_struct_var, 'post-covid_allsubj', working_dir)

#specify which columns of dataframe to use as covariates
X_test = all_v1[['agedays', 'sex']]

#make a matrix of response variables, one for each brain region
y_test = all_v1.loc[:, roi_ids]

#specify paths
training_dir = '{}/data/{}/ROI_models/'.format(working_dir, orig_struct_var)
...
Z_time2['participant_id'] = all_v1['participant_id'].copy()
Z_time2.reset_index(inplace=True, drop = True)

# Create dataframe to store variances
variance_time2 = pd.DataFrame()
variance_time2['participant_id'] = all_v1['participant_id'].copy()
...
for roi in roi_ids:
    ...
    dummy_cov_file_path_female, dummy_cov_file_path_male = \
        create_dummy_design_matrix(orig_struct_var, agemin, agemax, cov_file_te, spline_order, spline_knots, working_dir)

    plot_data_with_spline('Postcovid (Test) Data ', orig_struct_var, cov_file_te, resp_file_te, dummy_cov_file_path_female,
...
Z_time2.to_csv('{}/predict_files/{}/Z_scores_by_region_precovid_testset.txt'
...
plot_and_compute_zcores_by_gender(Z_time2, orig_struct_var, roi_ids)
plt.show()
mystop=1
